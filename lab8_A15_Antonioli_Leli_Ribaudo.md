...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // set timer
  LL_TIM_WriteReg(TIM3, ARR, 0xffff);                            // max value
  LL_TIM_WriteReg(TIM3, CR1, LL_TIM_ReadReg(TIM3, CR1) | 0x1);   // count enable
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if (LL_TIM_ReadReg(TIM3, CNT) >= 0x41A0){   // (84 MHz/5 MHz) = 16800 = 0x41A0
...
## 1.2 - Timer Output Compare Flag Polling 
...
## 1.4 - Timer Output Compare Function with Variable frequency
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (LL_TIM_ReadReg(TIM3, SR) & 0x2){ // if timer reaches threshold
		...
		if(LL_ADC_ReadReg(ADC1, SR) & (1 << 1)){ // read EOC bit: if ADC finishes conversion
			...
			int pot = LL_ADC_ReadReg(ADC1, DR) & 0xFFFF; // read pot current value
